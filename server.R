...
url <- "https://docs.google.com/spreadsheets/d/1yutijO8Kp-4xYJ1xyoybObS-srZABUgwctHBL3-rQVM/edit?usp=sharing" #data on google sheets, where anyone with this link can view/edit
  ...
  timecheck <- function(){
    ...
    # updatetime<-strptime("2020-08-24 8:00:00", "%Y-%m-%d %H:%M:%S")   # each time when publishing the new update, put in the date and time here the date and time it published.
    # displaytime <- format(updatetime,"%d %B %Y")
    displaytime
  }
  ...
  COVIDchosen <- reactive({
    if (input$COVIDoutc=='mortality') {
      dt<- read_sheet(url, sheet = "COVIDMortalitydata", col_names = FALSE)
      #dt<- read.csv("./COVIDMortalitydata.csv", header = FALSE)
      dt <- firstrowhead(dt)
      lb<- read_sheet(url, sheet = "COVIDMortalitylabel")
      #lb<- read.csv("./COVIDMortalitylabel.csv")
    } else if (input$COVIDoutc=='conversion') {
      dt<- read_sheet(url, sheet = "COVIDviralconversiondata", col_names = FALSE)
      #dt<- read.csv("./COVIDviralconversiondata.csv", header = FALSE)
      dt <- firstrowhead(dt)
      #lb<- read.csv("./COVIDviralconversionlabel.csv")
      lb<- read_sheet(url, sheet = "COVIDviralconversionlabel")
    } else {
      #dt<- read.csv("./COVIDSAEdata.csv",header = FALSE)
      dt<- read_sheet(url, sheet = "COVIDSAEdata",col_names = FALSE)
      dt <- firstrowhead(dt)
      #lb<- read.csv("./COVIDSAElabel.csv")
      lb<- read_sheet(url, sheet = "COVIDSAElabel")
    }
